 ...
 *
 * Patch: CSP admin warning is now based on real CSP violations (securitypolicyviolation),
 * instead of a GTM test script (which caused false positives due to adblockers/network).
 */
...
(function () {
  ...
  function loadConsent() {
    try {
      ...
      // Error loading consent data (ignored)
    }
  }
  ...
  function saveConsent() {
    try {
      ...
      document.cookie = `gdprConsent=${encodeURIComponent(
        JSON.stringify(consentData)
      )}; expires=${expireDate.toUTCString()}; path=/; SameSite=Lax`;
    } catch (e) {
      // Error saving consent data (ignored)
    }
  }
  ...
  function loadApprovedScripts() {
    if (typeof gdprScripts !== 'undefined') {
      ...
      const hasStatistics =
        consentData.statistics && gdprScripts.statistics && gdprScripts.statistics.trim() !== '';
      const hasMarketing =
        consentData.marketing && gdprScripts.marketing && gdprScripts.marketing.trim() !== '';
      const hasEmbedded =
        consentData.embedded_media &&
        gdprScripts.embedded_media &&
        gdprScripts.embedded_media.trim() !== '';
  ...
  function fetchScriptsViaAjax() {
    ...
    fetch(gdprAjax.url, {
      ...
      .catch(() => {
        // Failed to fetch scripts (ignored)
      });
  }
  ...
  function replaceYouTubeWithPlaceholder(iframe) {
    ...
    placeholder.setAttribute(
      'aria-label',
      `${title} - ${gdprTexts.youtubeLoadButton || 'Click to load YouTube video'}`
    );
    ...
    noticeElement.textContent =
      gdprTexts.youtubeNotice || 'This content is blocked until you accept embedded media cookies.';
  ...
  // ---------------------------------------------------------------------------
  // CSP ADMIN WARNING (PATCHED)
  // ---------------------------------------------------------------------------

  // Show admin warning if CSP blocks scripts (only when we have proof via CSP violation event)
  function showCspAdminWarning(details) {
    ...
    const violated = details && details.violatedDirective ? details.violatedDirective : '';
    const blocked = details && details.blockedURI ? details.blockedURI : '';
    ...
    const extra =
      violated || blocked
        ? `<hr style="border:none;border-top:1px solid #ffeeba;margin:0.75rem 0;" />
           <div style="font-size:0.9rem;">
             <div><strong>Violated directive:</strong> <code>${escapeHtml(violated)}</code></div>
             <div><strong>Blocked URI:</strong> <code>${escapeHtml(blocked)}</code></div>
           </div>`
        : '';

    warning.innerHTML =
      '<strong>GDPR Consent Manager:</strong> Externe scripts worden geblokkeerd door de Content Security Policy (CSP) van deze site. Voeg de benodigde domeinen toe aan de juiste directive(s) in je CSP (vaak <code>script-src</code>, maar soms ook <code>connect-src</code>, <code>img-src</code> en/of <code>frame-src</code>).<br><br>Voorbeeld:<br><code>script-src \'self\' https://www.googletagmanager.com https://www.youtube.com https://connect.facebook.net;</code><br>Zie de plugin documentatie voor meer info.' +
      extra;
  ...
  // Detect CSP blocking with hard evidence: securitypolicyviolation event
  function setupCspViolationListener() {
    if (!document.getElementById('wpadminbar')) return;

    document.addEventListener('securitypolicyviolation', function (e) {
      // Focus on likely relevant directives; expand if desired
      const vd = e.violatedDirective || '';
      const isRelevant =
        vd.startsWith('script-src') ||
        vd.startsWith('script-src-elem') ||
        vd.startsWith('connect-src') ||
        vd.startsWith('img-src') ||
        vd.startsWith('frame-src');
  ...
  // Run CSP listener only for admins
